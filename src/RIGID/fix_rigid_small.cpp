...
{
  double dtfm;
  int externalforcetorque = 1;
	//std::cout<<"final integrate "<<earlyflag<<"\n";
  //check(3);
  ...
  if (!earlyflag) compute_forces_and_torques();
  if (domain->dimension == 2) enforce2d();
  if (externalforcetorque) add_external_forces_and_torques();
...
/*---------------------------------------------------------------------
  adds external force and torques to each rigid body 
  ---------------------------------------------------------------------*/
...
void FixRigidSmall::setup_bodies_static()
{
  ...
  for (ibody = 0; ibody < nlocal_body; ibody++) {
    ...
    //std::cout<<ex[0]<<"\t"<<ex[1]<<"\t"<<ex[2]<<"\n";
    //std::cout<<ey[0]<<"\t"<<ey[1]<<"\t"<<ey[2]<<"\n";
...
------------------------------------------------------------------------- */
void FixRigidSmall::assign_variables(double *externalForcesToLAMMPS, double *externalTorqueToLAMMPS)
{
	external_forces=externalForcesToLAMMPS;
	external_torques=externalTorqueToLAMMPS;
}
